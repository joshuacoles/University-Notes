...
Assuming that $𝔼\left( Y right)$ exists, that is that
...
Note that if $X_{i}simBern\left( p right)$ then either $X_{i}=0$ or
...
An important application of this corollary is when $g\left( X right)=X$
