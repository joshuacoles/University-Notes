...
This limit can be interpreted as saying that the remainder $R$ in equation [[#^eq-2-6]] is much smaller that $x-c$ when $x\to c$. But can we get a more precise description of $R(x)$? Let us proceed assuming that $f$ has all the necessary derivatives, and let us apply De l’Hospital’s Rule, Theorem 2.32. We get that



$$
 $$ \being{aligned} \lim _{x\to c} \frac {R(x)}{(x-c)^2} &= \lim _{x\to c} \frac {f(x)-f(c)-f’(c)(x-c)}{(x-c)^2} \\ &= \lim _{x\to c} \frac {f’(x)-f’(c)}{2(x-c)} \\ &= \frac {1}{2} f^{\prime \prime }(c) . \end {aligned} 
$$
^eq-2-6
...
$$
 \being{align} \label {formula Rn} R_n(x) = \frac {1}{(n+1)!} f^{(n+1)}(c)(x-x_0)^{n+1} , \end{align} 
$$
^eq-2-6
...
$$
 \being{align*} R_n’(x) &= f’(x) - \sum _{j=1}^n \frac {f^{(j)}(x_0)}{(j-1)!} (x-x_0)^{j-1} , \\ R_n’(x_0) &= f’(x_0) - f’(x_0) =0. \end{align*} 
$$
^eq-2-7
...
$$
 \being{align*} R_n^{(k)}(x) &= f^{(k)}(x) - \sum _{j=k}^n \frac {f^{(j)}(x_0)}{(j-k)!} (x-x_0)^{j-k} , \\ R_n^{(k)}(x_0) &= f^{(k)}(x_0) - f^{(k)}(x_0) =0. \end{align*} 
$$
^eq-2-7
 We observe that
...
$$
 \being{align*} \frac {R_n(x)}{(x-x_0)^{n+1}} = \frac {R_n’(c_1)}{(n+1)(c_1-x_0)^n} \end{align*} 
$$
^eq-2-7
...
$$
 \being{align*} \frac {R_n’(c_1)}{(n+1)(c_1-x_0)^n} = \frac {R_n^{\prime \prime }(c_2)}{(n+1)n(c_2-x_0)^{n-1}} . \end{align*} 
$$
^eq-2-7
...
$$
 \being{align*} \frac {R_n(x)}{(x-x_0)^{n+1}} &= \frac {R_n^{k-1}(c_{k-1})}{(n+1)\cdot \ldots \cdot (n-k+3) (c_{k-1}-x_0)^{n-k+2}} \\ &= \frac {R_n^{k}(c_k)}{(n+1)\cdot \ldots \cdot (n-k+2) (c_k-x_0)^{n-k+1}} \end{align*} 
$$
^eq-2-7
...
$$
 \being{align*} \frac {R_n(x)}{(x-x_0)^{n+1}} = \frac {R_n^{(n)}(c_n) }{(n+1)! (c_n-x_0)} \end{align*} 
$$
^eq-2-7
...
$$
 \being{align*} \frac {R_n(x)}{(x-x_0)^{n+1}} = \frac {f^{(n+1)}(c) }{(n+1)! }, \end{align*} 
$$
^eq-2-7
 which is the same as [[#^eq-2-7]].
...
$$
 $$ \being{matrix} n=1, & \Sigma _n =2 , & d_n=2 \\ n=2 , &\Sigma _n =2.5 , & d_n=0.5 \\ n=3, &\Sigma _n = 2.66666666667, &d_n=0.116 \\ n=4, &\Sigma _n = 2.70833333333, &d_n=0.0228 \\ n=5, &\Sigma _n = 2.71666666667, & d_n=0.00378<10^{-2} \end {matrix} 
$$
^eq-2-7
