...
$$ \being{align} \label {max-min} $$ \being{aligned} &f(x) \leq f(p) , \quad \forall x\in [a,b] \\ &f(x) \geq f(q) , \quad \forall x\in [a,b]. \end {aligned} \end{align} $$
^eq-2-3
...
The remaining case is $p \notin (a,b)$ and $q \notin (a,b)$. This implies that $p \in \{a,b\}$ and $q\in \{a,b\}$. Since $f(a) = f(b)$ we deduce that $f(p) = f(q)$. From [[#^eq-2-4]] we obtain therefore
...
$$ \being{align*} \frac {f(b)-f(a)}{g(b)-g(a)} = \frac {f’(c)}{g’(c)}. \end{align*} $$
^eq-2-4
...
$$ \being{align*} f’(c) = \lim _{x\to c} \frac {f(x)-f(c)}{x-c} = \lim _{x\to c^+} \frac {f(x)-f(c)}{x-c} \end{align*} $$
^eq-2-4
...
Let $f\colon [0,1]\to \R$, $f(x) = x(2-x)$. Then, $f$ is continuous on $[0,1]$ and diﬀerentiable on $[[#^eq-0-1]]$, $f^\prime (x) = 2-2x > 0$, $x \in [[#^eq-0-1]]$. Hence, $f$ is strictly increasing on $[0,1]$.
...
$$ \being{align} \label {dlHospital} \frac {f(x)}{g(x)}=\frac {\frac {f(x)-f(c)}{x-c}}{\frac {g(x)-g(c)}{x-c}}. \end{align} $$
^eq-2-4
...
Since the limit of the right hand side of [[#^eq-2-5]] exists, so does that of the left hand side, and we obtain
...
$$ \being{align*} \frac {f(x_n)}{g(x_n)} = \frac {f’(c_n)}{g’(c_n)} . \end{align*} $$
^eq-2-5
...
$$ \being{align*} \lim _{n\to \infty }\frac {f(x_n)}{g(x_n)} = L. \end{align*} $$
^eq-2-5
  □
...
$$ \being{align*} \lim _{x\to 0} \frac {1-\cos (x)}{x^2} &= \lim _{x\to 0} \frac {\sin (x)}{2x} \end{align*} $$
^eq-2-5
 if the last limit exists. But


$$ \being{align*} \lim _{x\to 0} \frac {\sin (x)}{2x} = \frac {1}{2} \end{align*} $$
^eq-2-5
 by De L’Hospital’s rule, Theorem 2.30 or 2.32, and then


$$ \being{align*} \lim _{x\to 0} \frac {1-\cos (x)}{x^2} = \frac {1}{2}. \end{align*} $$
^eq-2-5
