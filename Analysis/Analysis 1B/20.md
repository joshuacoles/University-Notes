...
$$ \being{align} \label {approx by linear} \left |x-c\right | < \delta \quad \Longrightarrow \quad \left |f(x)-\left [f(c)+a(x-c)\right ]\right | \leq \varepsilon \left |x-c\right |. \end{align} $$
^eq-2-0
...
Formula [[#^eq-2-1]] is interpreted as saying that the tangent line at $(c,f(c))$ to the graph of $f$ is close to the graph of $f$, and the approximation gets better as we look closer to $c$.
...
$$ \being{align*} \lim _{h \to 0} \frac {f(c+h)-f(c)}{h} = \lim _{h \to 0} \frac {k-k}{h} = 0. \end{align*} $$
^eq-2-1
...
$$ \being{align*} \lim _{h\to 0} \frac {f(c+h)-f(c)}{h} &= \lim _{h\to 0} \frac {a(c+h)-ac}{h} = a. \end{align*} $$
^eq-2-1
...
$$ \being{align*} \frac {f(c+h)-f(c)}{h} = \frac {(c+h)^2-c^2}{h} = 2 c + h. \end{align*} $$
^eq-2-1
 Hence,
...
$$ \being{align*} \lim _{x\to c}f(x) - f(c) &= \lim _{x\to c}\frac {f(x)-f(c)}{x-c} \cdot \lim _{x\to c}(x-c)\\ &= f^\prime (c) \cdot 0 = 0. \end{align*} $$
^eq-2-1
...
$$ \being{align*} \lim _{h\to 0^+} \frac {f(h)-f(0)}{h} &= \lim _{h\to 0^+} \frac {h-0}{h} = 1 \quad \text {but}\\ \lim _{h\to 0^-} \frac {f(h)-f(0)}{h} &= \lim _{h\to 0^-} \frac {-h-0}{h} = -1, \end{align*} $$
^eq-2-1
...
$$ \being{eqnarray*} &&\frac {(f.g)(c+h) - (f.g)(c)}{h} \\ & &\qquad =\frac { f(c+h)g(c+h)-f(c)g(c)}{h}\qquad [\mbox {subtract and add }f(c+h)g(c)/h]\\ && \qquad = f(c+h)\frac {g(c+h)-g(c)}{h}+ g(c)\frac {f(c+h)-f(c)}{h}. \end{eqnarray*} $$
^eq-2-1
...
$$ \being{eqnarray*} &&\lim _{h\to 0} \frac {(f\cdot g)(c+h)-(f\cdot g)(c)}{h} \\ &&\quad = \quad \left (\lim _{h\to 0} f(c+h)\right )\left (\lim _{h\to 0} \frac {g(c+h)-g(c)}{h}\right ) + g(c) \lim _{h\to 0}\frac {f(c+h) - f(c)}{h} \\ &&\quad = \quad f(c) \cdot g^\prime (c) + g(c)f^\prime (c), \end{eqnarray*} $$
^eq-2-1
