...
Proof. Suppose that $L_1>L_2$. Let $\varepsilon = \frac {L_1-L_2}{2}>0$. Then by definition there exists $\delta _1>0$ such that
...
Proof. Let $\varepsilon := L-M$, which is positive. Applying Definition 1.3, there exists $\delta > 0$ such that
...
Proof. Suppose otherwise; i.e. $L>M$. Then, the conditions of Lemma 1.8 are satisfied and there exists $\delta >0$ such that
