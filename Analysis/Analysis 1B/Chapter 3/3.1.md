...
In this chapter we study the Riemann integral for functions $f: [a,b]\to \R$ ($a\leq b$). Under certain hypothesis, we will define a number $\int _a^b f$, so that, if $f(x) \geq 0$ for all $x\in [a,b]$, $\int _a^b f$, can be interpreted as the area of the region
...
Let $f: [a,b]\to \R$ be bounded and $P = \{x_0, \dotsc , x_n\}$ be a subdivision of $[a, b]$ with $P = \{x_0, x_1, \dotsc , x_n\}$ and $a = x_0 < x_1 < \dotsb < x_n = b$. The _lower Riemann sum_ of $f$ for $P$, denoted $L(f, P)$, is defined as
...
Let $f: [a, b] \rightarrow \mathbb {R}$ be bounded, and let $P$ be a subdivision of $[a, b]$. Then
...
Let $f: [a,b]\to \R$ be bounded and $P,Q$ be subdivisions of $[a,b]$. Then
...
Let $f: [a,b]\to \R$ be bounded and $P,Q$ be subdivisions of $[a,b]$ with $P \subset Q$. Then
...
$$
\begin{align} L(f,Q)-L(f,P) & = \, ( \inf _{[x_{k-1},q]} f) \, ( q-x_{k-1} ) + \, ( \inf _{[q,x_{k}]} f) \, ( x_k- q ) \\ \nonumber & \quad - \, ( \inf _{[x_{k-1},x_{k}]} f) \, ( x_k- x_{k-1}) \end{align}
$$
...
We can state Lemma 3.6 as: Let $f: [a,b]\to \R$ be bounded and $P,Q$ be subdivisions of $[a,b]$. If $Q$ is a refinement of $P$, then
