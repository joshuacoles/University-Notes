...
$$
 \being{align} \label {def:uniform-continuity} \forall \varepsilon >0 \quad \exists \delta >0 \quad \forall x_1,x_2\in I \quad |x_1-x_2|<\delta \Rightarrow |f(x_1)-f(x_2)|<\varepsilon . \end{align} 
$$
^eq-3-6
...
2. In general the reciprocal is not true. For example, $f(x) = \frac {1}{x}$, $f\colon [[#^eq-0-1]]\to \R$ is not uniformly continuous on $[[#^eq-0-1]]$. Similarly $f(x) = e^x$, $f\colon \R \to \R$ is not uniformly continuous on $\R$.
...
$$
 \being{align} \label {def:continuity-in-I} $$ \being{aligned} & \forall x_1 \in I \quad \forall \varepsilon >0 \quad \exists \delta >0 \quad \\ & \forall x_1,x_2\in I \quad |x_1-x_2|<\delta \Rightarrow |f(x_1)-f(x_2)|<\varepsilon . \end {aligned} \end{align} 
$$
^eq-3-7
 The diﬀerence between [[#^eq-3-7]] and [[#^eq-3-8]], is that in the latter, $\delta$ normally depends on $x_1$ and $\varepsilon$. Although $\delta$ is not uniquely deﬁned, for a ﬁxed $\varepsilon >0$ we may choose $\delta = \delta (x_1)$. It may happen that $\displaystyle \inf _{x_1\in I} \delta (x_1) = 0$. On the contrary, property [[#^eq-3-7]] says that one can choose a ﬁxed (or uniform) $\delta >0$ independent of $x_1$.

Let us show that indeed $\displaystyle \inf _{x_1\in I} \delta (x_1) = 0$ can happen. Consider $f \colon [[#^eq-0-1]] \to \R$, $f(x) = \frac {1}{x}$. Let $\varepsilon >0$ be given and let us try to ﬁnd $\delta (x_1)>0$ with the property



$$
 \being{align*} \forall x_1,x_2\in [[#^eq-0-1]] \quad |x_1-x_2|<\delta \Rightarrow |f(x_1)-f(x_2)|<\varepsilon . \end{align*} 
$$
^eq-3-8
...
$$
 \being{align*} \delta (x_1) = \min \Bigl ( \frac {1}{\frac {1}{x_1} - \varepsilon } - x_1, x_1 - \frac {1}{\frac {1}{x_1}+\varepsilon } \Bigr ) = \frac {1}{\frac {1}{x_1} - \varepsilon } - x_1 . \end{align*} 
$$
^eq-3-8
...
$$
 \inf _{x_1\in [[#^eq-0-1]] } \delta (x_1) = 0. 
$$
...
$$
 \being{align*} \lim _{x_1\to 0^+ } \delta (x_1) &= \lim _{x_1\to 0^+ } \Bigl ( \frac {1}{\frac {1}{x_1} - \varepsilon } - x_1 \Bigr ) \\ &= \lim _{x_1\to 0^+ } \frac {\varepsilon x_1^2}{1-\varepsilon x_1} =0. \end{align*} 
$$
^eq-3-8
...
$$
 \being{align*} $$ \being{aligned} & \exists \varepsilon >0 \quad \forall \delta >0 \quad \exists x_1,x_2\in [a,b] \text { such that } \\ & |x_1-x_2|<\delta \text { and } |f(x_1)-f(x_2)|\geq \varepsilon . \end {aligned} \end{align*} 
$$
^eq-3-8
...
$$
 \label {1} \left \{ $$ \being{aligned} & \forall \delta >0 \quad \exists x_1,x_2\in [a,b] \text { such that } \\ & |x_1-x_2|<\delta \text { and } |f(x_1)-f(x_2)|\geq \varepsilon . \end {aligned} \right . 
$$
^eq-3-8


For any $n\in \N$ we apply [[#^eq-3-9]] with $\delta = \frac {1}{n}$ and ﬁnd points $x_n$, $z_n \in [a,b]$ such that
...
This and the convergence [[#^eq-3-10]] implies that $x= z$.
...
We let $j\to \infty$ in this inequality and use the convergence [[#^eq-3-11]] to ﬁnd that
...
Given the $\delta >0$, from the uniform continuity [[#^eq-3-12]], we let $n\in \N$ be such that $\frac {b-a}{n}<\delta$. Then we deﬁne the uniform subdivision of $[a,b]$
...
For $y,z \in I_j$ we have $|y-z|\leq \frac {b-a}{n}<\delta$. The uniform continuity [[#^eq-3-12]] implies that
...
Replacing this in the expression for $U(f,P_n)-L(f,P_n)$ [[#^eq-3-13]] we obtain
