...
Let $f:[[#^eq-0-1]]\to \R$, $f(x)=1/x$. Show that $f$ is continuous on $[[#^eq-0-1]]$.

###### Solution

. 

Let $c\in [[#^eq-0-1]]$. Given $\varepsilon >0$, suppose that $x\in [[#^eq-0-1]]$ and $|x-c|<\delta$ for some $\delta >0$ to be determined. We have
...
Then, if $x \in [[#^eq-0-1]]$ and $|x-c|<\delta$ we have
...
Thus $f$ is continuous at $c$. As $c\in [[#^eq-0-1]]$ was arbitrary, $f\colon [[#^eq-0-1]]\to \R$ is continuous.
...
$$
 \being{align} \label {11b} |f(x_\delta )-f(c)| \geq \varepsilon . \end{align} 
$$
^eq-1-5
...
Let us write $x_n = x_{\delta _n}$. Since $|x_n-c|<\frac {1}{n}$ we see that $\displaystyle \lim _{n\to \infty } x_n = c$. By (b) we get that $\lim _{n\to \infty }f(x_n) = f(c)$. Therefore there exists $N\in \N$, such that for $n\geq N$ $|f(x_n)-f(c)| < \varepsilon$. But this contradicts [[#^eq-1-6]] and this ﬁnishes the proof.
...
By Deﬁnition 1.18, given $\varepsilon >0$, there exists $\delta >0$ such that
...
Deﬁnition 1.24 (Composition of functions). 
...
$$
 \being{align*} (f\circ g)(x) & =3(1+\sin (x))^2+5 , \quad x \in \R , \\ (g\circ f)(y) &=1+\sin (3y^2+5) , \quad y \in \R . \end{align*} 
$$
^eq-1-6
...
$$
 \being{align} \label {cont-f} \forall y \in E, \quad |y-g(c)|<\eta \quad \Longrightarrow \quad |f(y)-f(g(c))|<\varepsilon . \end{align} 
$$
^eq-1-6
...
Then for $x\in D$ such that $|x-c|<\delta$ we have that $g(x) \in E$ and $| g(x) - g(c)|<\eta$. Therefore we can use [[#^eq-1-7]] with $y = g(x)$ and we get
