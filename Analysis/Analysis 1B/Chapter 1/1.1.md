...
Here we define precisely the notion of the limit of a function and study its properties. Suppose that $f : \R \to \R$ and $c\in \R$. The idea is that the statement “the limit of $f(x)$ is equal to $L$ as $x$ tends to $c$”, where $L$ is a real number, means that the difference $f(x) - L$ can be made arbitrarily small provided that $x$ is sufficiently close to $c$, but not equal to $c$. We write this as $\displaystyle \lim_{x \to c} f(x)=L$.
