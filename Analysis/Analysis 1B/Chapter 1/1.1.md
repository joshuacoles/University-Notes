...
Here we define precisely the notion of the limit of a function and study its properties. Suppose that $f: \R \to \R$ and $c\in \R$. The idea is that the statement “the limit of $f(x)$ is equal to $L$ as $x$ tends to $c$”, where $L$ is a real number, means that the difference $f(x) - L$ can be made arbitrarily small provided that $x$ is sufficiently close to $c$, but not equal to $c$. We write this as $\displaystyle \lim _{x\to c} f(x)=L$.

### Example 1.1

Let $f: (-\frac {\pi }{2},0)\cup (0,\frac {\pi }{2})\to \R$ be defined by $f(x) = \frac {\tan (x)}{x}$. Using a calculator we get that
...
Let $f: D \to \R$ and $c \in \R$, so that $D$ is a punctured neighbourhood of $c$, and let $L \in \R$. We say that
...
%%%%
$$
\begin{align} & \forall \varepsilon >0 \quad \exists \delta >0 \quad \forall x \in D, \quad 0 < |x-c| < \delta \ \Longrightarrow \ |f(x) - L| < \varepsilon . \end{align}
$$
...
%%\label {non-isolated}%%
$$
\begin{align}  \forall \delta >0 \quad [(c-\delta ,c)\cup (c,c+\delta ) ] \cap D \not =\emptyset . \end{align}
$$
...
Here is the result that justifies the uniqueness of the limit.

## Lemma 1.5 (Uniqueness of limits).

Let $f: D \to \R$, $c\in \R$ with $D$ a punctured neighbourhood of $c$. Suppose that $\displaystyle \lim _{x\to c} f(x) = L_1$ and $\displaystyle \lim _{x\to c} f(x) = L_2$ for some $L_1,L_2\in \R$. Then $L_1 = L_2$.
...
For each of the following $f: \R \to \R$, $c$ and $L$, show that $\displaystyle \lim _{x\to c}f(x)=L$.
...
Let $f: D\to \R$ and $c, L \in \R$. Suppose that $D$ is a punctured neighbourhood of $c$. Then, the following statements are equivalent:
...
Let $f: D\to \R$ where $D\subset \R$, and let $c, L, M \in \R$. Suppose that $D$ contains a punctured neighbourhood of $c$ and that $\displaystyle \lim _{x\to c}f(x)=L>M$. Then there exists $\delta >0$ such that
...
Let $f: D\to \R$, let $c, L, M \in \R$, and let $d>0$. Suppose that $D$ is a punctured neighbourhood of $c$, $(c,c+d) \subset D$, $\displaystyle \lim _{x\to c}f(x)=L$, and that

%%\seteqnumber{0}{1.}{3}%%

$$
\forall x \in D, \quad 0<x-c<d \Longrightarrow f(x)\leq M.
$$
