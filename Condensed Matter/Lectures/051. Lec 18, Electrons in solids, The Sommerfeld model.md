...
Instead we extend the domain of the problem from $[0, L]$ to $\R$, and  modify the boundary conditions such that $\psi(x) = \psi(x + L)$, ie the function is periodic by $L$. This leads to solutions in the form,
...
modify the boundary conditions of the problem, going from $[0, L]$ to $\R$, and 