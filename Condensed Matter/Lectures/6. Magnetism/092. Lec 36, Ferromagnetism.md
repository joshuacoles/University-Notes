...
which has a characteristic temperature of $0.1\unit{K}$, implying that any order generated through this interaction will not survive temperature above that. This is in conflict with observed [[Curie Temperatures]] shown below
...
- Paramagnetism happens when normally randomly orientated.
- Also applying a field changes orbital motion and thus [[Diamagnetism]].
- Now permanent magnetism
- This is due to magnetic order
	- At low temps some materials exhibit finite $\vM$. Spontaneous order
	- Examples of order
		- Ferromagnetic order
			- All moments contribute equally.
		- Ferrimagnetic order
			- Some cancellation but overall finite
		- antiferromagnetic
			- Regular order, all cancel
- But why
	- Interactions between dipoles cannot explain Spontaneous ordering
		- $10^{-24}\unit{J} \sim 0.1\unit{K}\,k_B$, hence this is weak against thermal disorder.
		- Against what is observed ($T_C$ table)
	- Some maths, Intro
		- State is anti-symmetric in swapping electrons ($\psi(r_1, s_1, r_2, s_2) = -\psi(r_2, s_2, r_1, s_1)$)
		- Parallel spins: Leads to zero for $\psi(r_1, up, r_1, up) = -\psi(r_1, up, r_1, up) = 0$
		- Antiparallel spins: $\psi(r_1, up, r_1, down) = -\psi(r_1, down, r_1, up)$. Does not lead to vanishing per-se.
		- Tho the spin doesnt effect the [[Electrostatic potential|Electrostatic Interaction]] it does affect the position dist so leads to diff $U = bra U_e ket$
	- More stuff
		- More generally exchange energy of a solid can be approximated with the [[Heisenberg Hamiltonian]]
		- Mean feild approx
		- Include magnetiation
		- This exchange interaction looks like a magneitc field
		- Effects of this
			- Add exchange magnetic field to the result with tanh from before
				- Note this is non trivial in $\vM$
			- High $T$ approximation
				- [[Curie's Constant]]
				- $T_C = \lambda C$, $\lambda$ from exchange field.
				- [[Curie-Weiss Law]]
				- $\chi = {C\over T-T_C}$ 
				- High $T$ => Paramagnetic
			- No $\B_0$ case
				- Can reach saturation or some shit
				- Doing lots of maths
				- Spontaneous mag at $T < T_C$ up until the saturation thingy
- Domains and shit
