...
Casting the function pointer to remove the specialisation of context parameter.

#### Plotting

Plotting these with Julia we obtain,
...
```c
// Intensity function, note this is as a proportion of the maximum / initial intensity.  
double I(double r) {  
    double focalRatio = 10.0;  
 double preFactor = (1 / focalRatio) \* M\_PI / lambda;  
 double x = preFactor \* r;  
 // Discuss the use of abs wrt https://en.wikipedia.org/wiki/Airy\_disk#Mathematical\_formulation  
 double v = 2.0 \* J(1, fabs(x)) / fabs(x);  
