...
Let $\lambda$ be an eigenvalue of $f$ . Note that $A$ has at least an eigenvalue, because its characteristic polynomial has at least one root in $\C$ . Let $f_{\lambda}$ be the linear operator
